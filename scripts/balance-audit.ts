...
const levelHpMult = (levelIdx: number) =>
  LEVELS[levelIdx]?.enemyHpMultiplier ?? 1;
const levelDmgMult = (levelIdx: number) =>
  LEVELS[levelIdx]?.enemyDmgMultiplier ?? 1;
...
function enemySpeed(type: string, wave: number): number {
  return (
    ENEMY_CONFIGS[type as keyof typeof ENEMY_CONFIGS].moveSpeed * diffFor(wave)
  );
}
...
function bossHp(levelIdx: number, wave: number): number {
  ...
  return (
    cfg.maxHealth * bossDiffFor(wave) * levelHpMult(levelIdx) * HP_GLOBAL_MULT
  );
}
...
function calcWeaponDps(
  ...
  const w = WEAPONS[weaponId];
  const perShot =
    weaponBaseDmg(weaponId, level) * (atkPower / 10) * CRIT_EXPECT;
  return (
    perShot *
    w.attackSpeed *
    atkSpeedMult *
    (WEAPON_HIT_TARGETS[weaponId] ?? 1) *
    HIT_RATE
  );
}
...
function waveSpawnCount(wave: number, maxEnemies = 120): number {
  const interval = Math.max(
    ...
    GameConfig.WAVE.spawnIntervalBase - wave * 50,
  );
...
const passiveEventsGroup =
  WEAPONS.default_gun.attackSpeed *
  PASSIVE_BUILD.attackSpeedMult *
  HIT_RATE *
  1.5;
const passiveEventsSingle =
  WEAPONS.default_gun.attackSpeed * PASSIVE_BUILD.attackSpeedMult * HIT_RATE;
...
function calcPassiveRows(): PassiveAuditRow[] {
  ...
  const dotDps = (lv: number, targets: number) =>
    0.15 * lv * (0.15 * lv) * p * (7 / 3) * targets;
  const bounceDps = (lv: number, events: number) =>
    0.7 * p * Math.min(lv, GROUP_ENEMIES - 1) * events;
  const chainDps = (lv: number, events: number) =>
    0.1 * lv * 0.6 * p * Math.min(lv, GROUP_ENEMIES - 1) * events;
  ...
  return [
    ...
    },
    {
      ...
      name: "灼烧",
      group: [
        dotDps(1, GROUP_ENEMIES),
        dotDps(3, GROUP_ENEMIES),
        dotDps(5, GROUP_ENEMIES),
      ],
...
function buildHtml(rows: {
  ...
  const barChart = (
    items: Array<{ name: string; lv1: number; lv3: number; max: number }>,
  ) => {
...
${lineChart([
  {
    label: "每波金币收入",
    values: economy.map((e) => e.coins),
    color: "#e8c33d",
  },
  {
    label: "商店最低价",
    values: economy.map((e) => e.shopMin),
    color: "#5b8fb0",
  },
  {
    label: "商店最高价(传说)",
    values: economy.map((e) => e.shopMax),
    color: "#e05252",
  },
])}
...
${lineChart([
  {
    label: "草原",
    values: levelCompare.map((c) => c.meadow),
    color: "#7bd88f",
  },
  {
    label: "废墟(×1.1+嗜血)",
    values: levelCompare.map((c) => c.ruins),
    color: "#e8a33d",
  },
  {
    label: "冰原(×1.3+霜蚀)",
    values: levelCompare.map((c) => c.tundra),
    color: "#6db3f2",
  },
])}
...
function main(): void {
  ...
  const levelCurve = Array.from({ length: 30 }, (_, i) => ({
    level: i + 1,
    exp: expToNext(i + 1),
  }));
  ...
  const economy = WAVES.map((w) => {
    ...
    return {
      wave: w,
      coins: Math.round(kills * avgCoinsPerKill(w)),
      shopMin: 20,
      shopMax: 80,
    };
  });
  ...
  BUILDS.forEach((b) => {
    ...
    BOSS_WAVES.forEach((w) => {
      ...
      if (wavesNeeded >= 3)
        ...
        conclusions.push(
          `ℹ️ 草原 ${w} 波 Boss 用「${b.name}」约 ${wavesNeeded} 波击杀，需边躲边输出（可接受）`,
        );
    });
  });
  // 1b) Boss 单次攻击力（接触伤害，未乘关卡）——评估"挨一下掉多少"
  const bossAtkInfo = BOSS_WAVES.map(
    (w) => `wave${w}:${fmt(ENEMY_CONFIGS.boss.attackPower * diffFor(w))}`,
  ).join(" ");
  conclusions.push(
    `ℹ️ 草原 Boss 接触伤害（随波次）：${bossAtkInfo}——玩家 100 血时约 2-3 下倒，需走位/护盾`,
  );
  ...
  conclusions.push(
    `ℹ️ 无尽 50 波草原 Boss 血量 ${fmt(boss50)}（2.2^9），需 50w+ 级别 DPS——验证后期 build 是否跟得上`,
  );
  // 6) 被动定位（成型 build）
  ...
  WAVES.forEach((w) => {
    console.log(
      String(w).padEnd(6) +
        types.map((t) => fmt(enemyHp(t, w)).padEnd(10)).join(""),
    );
  });
  ...
  weaponDpsTable
    ...
    .forEach((w) => {
      console.log(
        `${w.name.padEnd(8)}\t${fmt(w.lv1, 1)}\t${fmt(w.lv3, 1)}\t${fmt(w.max, 1)}`,
      );
    });
  ...
  console.log(
    "经验\t" +
      [1, 2, 3, 5, 8, 12, 16, 20, 25, 30]
        .map((l) => fmt(expToNext(l)))
        .join("\t"),
  );

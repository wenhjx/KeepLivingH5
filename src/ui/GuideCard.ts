...
export class GuideCard {
  ...
  constructor(
    ...
    this.container = scene.add
      .container(0, 0)
      .setDepth(Layers.GUIDE)
      .setAlpha(0); // 最高层级：成就/引导提示必须浮于结算遮罩之上
  ...
  private build(): void {
    ...
    if (this.config.iconTexture) {
      const icon = this.scene.add
        .image(0, 0, this.config.iconTexture)
        .setOrigin(0.5, 0.5);
      ...
      iconText.setPosition(
        contentX + 14,
        -this.cardHeight / 2 + this.padding + 16,
      );
    ...
    const description = createUIText(
      this.scene,
      0,
      0,

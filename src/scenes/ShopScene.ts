...
export class ShopScene extends Phaser.Scene {
  ...
  private aiShoppingStep(): void {
    ...
    const candidates = this.cardRefs
      ...
      .sort(
        (a, b) =>
          (ShopScene.RARITY_RANK[b.item.rarity] ?? 0) -
            (ShopScene.RARITY_RANK[a.item.rarity] ?? 0) ||
    ...
    if (candidates.length > 0) {
      ...
      this.time.delayedCall(250 + Math.random() * 350, () =>
        this.aiShoppingStep(),
      );
      return;
    }
    ...
    if (canFreeRefresh || canPayRefresh) {
      this.tryRefresh();
      this.time.delayedCall(350 + Math.random() * 300, () =>
        this.aiShoppingStep(),
      );
      return;
    }
  ...
  private getPlayer(): Player | undefined {
    return (this.scene.get("GameScene") as any)?.getPlayer?.() as
      Player | undefined;
  }
  ...
  private renderStock(): void {
    ...
    const totalWidth =
      this.stock.length * this.cardWidth +
      (this.stock.length - 1) * this.cardSpacing;
  ...
  private createRefreshButton(): void {
    ...
    this.refreshText = createUIText(
      this,
      x + 85,
      y + 22,
      this.getRefreshLabel(),
      {
        fontSize: "16px",
        color: "#ffffff",
        fontStyle: "bold",

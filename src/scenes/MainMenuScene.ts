...
export class MainMenuScene extends Phaser.Scene {
  ...
  private qualityTexts: Record<QualityLevel, Phaser.GameObjects.Text> =
    {} as Record<QualityLevel, Phaser.GameObjects.Text>;
  ...
  create(data?: { openLevelSelect?: boolean }): void {
    ...
    this.createMenuButton(centerX, buttonY, "开始游戏", () =>
      this.scene.start("CharacterSelectScene"),
    );
    this.createMenuButton(centerX, buttonY + buttonSpacing, "继续游戏", () =>
      this.continueGame(),
    );
    this.createMenuButton(centerX, buttonY + buttonSpacing * 2, "设置", () =>
      this.openSettings(),
    );
    this.createMenuButton(centerX, buttonY + buttonSpacing * 3, "🏅 成就", () =>
  ...
  private createLevelSelectOverlay(): void {
    ...
    LEVELS.forEach((lv, i) => {
      ...
      if (unlocked) {
        ...
        infoBtn.on("pointerover", () =>
          infoBtn.setStyle({ backgroundColor: "#2a2a55" }),
        );
    ...
    codexBtn.on("pointerover", () =>
      codexBtn.setStyle({ backgroundColor: "#2a2a55" }),
    );
    codexBtn.on("pointerout", () =>
      codexBtn.setStyle({ backgroundColor: "#1a1a35" }),
    );
  ...
  private updateLevelPreview(level: LevelConfig): void {
    ...
    for (let r = 0; r < this.levelPreviewRows.length; r++) {
      ...
      if (item) {
        ...
        row.note
          .setText(
            item.note.length > 12 ? item.note.slice(0, 12) + "…" : item.note,
          )
          .setVisible(true);
      } else {
  ...
  private createSettingsOverlay(): void {
    ...
    this.settingsOverlay.add(
      createUIText(this, cx - 170, musicY, "音乐音量", labelStyle).setOrigin(
        0,
        0.5,
      ),
    );
    this.musicVolText = createUIText(
      this,
      cx - 20,
      musicY,
      `${this.musicVolume}%`,
    ...
    this.settingsOverlay.add(
      this.createSmallButton(cx + 40, musicY, "−", () => this.adjustMusic(-10)),
    );
    this.settingsOverlay.add(
      this.createSmallButton(cx + 90, musicY, "+", () => this.adjustMusic(10)),
    );
    // ---------- 音效音量 ----------
    const sfxY = cy - panelH / 2 + 146;
    this.settingsOverlay.add(
      createUIText(this, cx - 170, sfxY, "音效音量", labelStyle).setOrigin(
        0,
        0.5,
      ),
    );
    ...
    this.settingsOverlay.add(
      this.createSmallButton(cx + 40, sfxY, "−", () => this.adjustSfx(-10)),
    );
    this.settingsOverlay.add(
      this.createSmallButton(cx + 90, sfxY, "+", () => this.adjustSfx(10)),
    );
    // ---------- 画质 ----------
    const qualityY = cy - panelH / 2 + 196;
    this.settingsOverlay.add(
      createUIText(this, cx - 170, qualityY, "画质", labelStyle).setOrigin(
        0,
        0.5,
      ),
    );
    ...
    qLabels.forEach((lvl, i) => {
      const text = this.createSmallButton(
        cx - 40 + i * 70,
        qualityY,
        qNames[lvl],
        () => this.setQuality(lvl),
    ...
    this.settingsOverlay.add(
      createUIText(this, cx - 170, muteY, "静音", labelStyle).setOrigin(0, 0.5),
    );
    ...
    this.settingsOverlay.add(
      createUIText(this, cx - 170, showFpsY, "显示FPS", labelStyle).setOrigin(
        0,
        0.5,
      ),
    );
    const gmFps = GameManager.getInstance();
    this.showFpsText = createUIText(
      this,
      cx,
      showFpsY,
      gmFps.showFps ? "开" : "关",
    ...
    this.settingsOverlay.add(
      createUIText(this, cx - 170, fullscreenY, "全屏", labelStyle).setOrigin(
        0,
        0.5,
      ),
    );
  ...
  private createSmallButton(
    ...
    const btn = createUIText(this, x, y, label, {
      ...
      .setInteractive({ useHandCursor: true });
    btn.on("pointerover", () =>
      btn.setStyle({ color: UIColors.textBright, backgroundColor: "#353555" }),
    );
    btn.on("pointerout", () =>
      btn.setStyle({
        color: UIColors.text,
  ...
  private toggleShowFps(): void {
    ...
    this.showFpsText.setColor(
      gm.showFps ? UIColors.accent : UIColors.textBright,
    );
  }

...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.cameras.main.setBackgroundColor(
      "#" + UIColors.sceneBg.toString(16).padStart(6, "0"),
    );
    ...
    ids.forEach((id, i) => {
      ...
      this.createCard(
        ...
        cardW,
        cardH,
        id === GameManager.getInstance().getActiveCharacterId(),
      );
    });
    ...
    const playBtn = createUIButton(
      this,
      width - 16,
      height - 16,
      "▶ 开始冒险",
      () => this.scene.start("MainMenuScene", { openLevelSelect: true }),
      {
  ...
  private createCard(
    ...
    const draw = () => {
      ...
      gfx.lineStyle(
        selected ? 3 : 1,
        selected ? UIColors.accentDim : UIColors.cardBorder,
        1,
      );
    ...
    if (selected && !isNarrow) {
      texts.push(
        createUIText(this, 0, h / 2 - 18, "✓ 当前角色", {
          fontSize: UIFonts.small,
          color: UIColors.accent,
        }).setOrigin(0.5),
      );

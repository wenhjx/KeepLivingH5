...
export interface LevelConfig {
  ...
  /** 本关独有敌人（每波恒定权重，第 2 波起出现） */

  exclusiveEnemies?: Partial<Record<EnemyType, number>>;

  /** 关卡图鉴：选关界面展示的本关敌人说明 */

  enemyPreview?: Array<{ type: EnemyType; note: string }>;
...
export const LEVELS: LevelConfig[] = [
...

  {
    ...
    bossType: 'boss',
    enemyPreview: [
      { type: 'normal', note: '基础步兵，成群出现' },
      { type: 'fast', note: '移动迅速，拉开距离再输出' },
      { type: 'tank', note: '皮糙肉厚，需要高伤害武器' },
      { type: 'ranged', note: '远程压制，注意走位' },
      { type: 'boss', note: '关底守门人，注意阶段技能' },
    ],
  },
...

  {
    ...
    exclusiveEnemies: { corrupt_zombie: 28 },

    enemyPreview: [
      { type: 'corrupt_zombie', note: '废墟特有，移动迟缓但更耐打' },

      { type: 'suicider', note: '近身即爆，保持距离' },

      { type: 'splitter', note: '击杀后分裂，优先处理' },

      { type: 'summoner', note: '召唤小怪，先集火它' },

      { type: 'boss_summoner', note: '持续召唤魔像群，清场优先' },
    ],
...

  {
    ...
    exclusiveEnemies: { frost_zombie: 28 },

    enemyPreview: [
      { type: 'frost_zombie', note: '冰原特有，移速极快' },

      { type: 'tank', note: '冰甲加厚，血量更高' },

      { type: 'ranged', note: '远程压制，保持机动' },

      { type: 'shielded', note: '正面减伤，绕背输出' },

      { type: 'boss_barrage', note: '环形+扇形弹幕，找缝隙走位' },
    ],
...
export const getLevelById = (id: string): LevelConfig | undefined => LEVELS.find((l) => l.id === id);

/** 按序号取配置（越界返回最后一关） */

export const getLevelByIndex = (index: number): LevelConfig => LEVELS[Math.max(0, Math.min(index, LEVELS.length - 1))];

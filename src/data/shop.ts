...
const CONSUMABLE_ITEMS: ShopItem[] = [
  ...
  {
    id: "consumable_magnet",
    name: "大磁铁",
    icon: "🧲",
    desc: "存入物品栏，使用后 8 秒内全屏吸附金币与经验",
    price: 30,
    rarity: "rare",
    kind: "consumable",
    timing: "immediate",
...
export function generateShopStock(player: Player): ShopItem[] {
  ...
  const premium =
    shuffle(highPool).find(
      (it) =>
        it.kind === "weapon" &&
        !player.hasWeapon?.(it.upgradeId!.replace("weapon_", "")),
    ) || shuffle(highPool)[0];

  // ---------- 常规位：智能补货 ----------
  const lowHealth = player.getHealth() < player.getMaxHealth() * 0.5;
  const weaponPool = WEAPON_ITEMS.filter(
  ...
  const regularPool = shuffle([
    ...
    ...weaponPool.filter(
      (w) => !player.hasWeapon?.(w.upgradeId!.replace("weapon_", "")),
    ),
    ...weaponPool.filter(
      (w) => !player.hasWeapon?.(w.upgradeId!.replace("weapon_", "")),
    ),
    ...
    ...CONSUMABLE_ITEMS.filter(
      (it) =>
        it.kind !== "consumable" ||
        (it.id !== premium?.id &&
          (!lowHealth || it.id !== "consumable_big_heal")),
    ),
    // 低血量时大血包加权
    ...(lowHealth
      ? [CONSUMABLE_ITEMS.find((c) => c.id === "consumable_big_heal")!]
      : []),
    ...(lowHealth
      ? [CONSUMABLE_ITEMS.find((c) => c.id === "consumable_big_heal")!]
      : []),
  ...
  const filler = shuffle(
    SHOP_POOL.filter(
      (it) =>
        isPurchasable(it, player) &&
        !regulars.some((r) => r.id === it.id) &&
        it.id !== premium?.id,
    ),
  );
  ...
  if (regulars.length < 3) {
    const fallbacks = shuffle(FALLBACK_UPGRADES)
      ...
      .filter(
        (it) => !regulars.some((r) => r.id === it.id) && it.id !== premium?.id,
      );

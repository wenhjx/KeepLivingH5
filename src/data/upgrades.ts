 ...
 */
export function passiveDescForLevel(
  id: string,
  level: number,
  fallback: string,
): string {
...
export function passiveLevelTexts(
  id: string,
  level: number,
): PassiveLevelDesc | null {
  switch (id) {
    case "passive_regen":
      return {
        current: `每秒恢复 ${1 + level} 点生命`,
        next: `每秒恢复 ${2 + level} 点生命`,
      };
    case "passive_thorns":
      return {
        current: `受击反弹 ${20 + level * 5}% 伤害`,
        next: `受击反弹 ${25 + level * 5}% 伤害`,
      };
    case "passive_exp_boost":
      return {
        current: `获得经验 +${25 + level * 10}%`,
        next: `获得经验 +${35 + level * 10}%`,
      };
    case "passive_gold_boost":
      return {
        current: `获得金币 +${50 + level * 10}%`,
        next: `获得金币 +${60 + level * 10}%`,
      };
    case "passive_lifesteal":
      return {
        current: `攻击回复 ${level * 3}% 伤害`,
        next: `攻击回复 ${(level + 1) * 3}% 伤害`,
      };
    case "passive_freeze":
      return {
        current: `冰冻概率 ${level * 8}%（减速 2 秒）`,
        next: `冰冻概率 ${(level + 1) * 8}%（减速 2 秒）`,
      };
    case "passive_burn":
      return {
        current: `点燃概率 ${level * 15}%`,
        next: `点燃概率 ${(level + 1) * 15}%`,
      };
    case "passive_chain":
      return {
        current: `闪电链概率 ${level * 10}%`,
        next: `闪电链概率 ${(level + 1) * 10}%`,
      };
    case "passive_bounce":
      return {
        current: `弹射 ${level} 次（70% 伤害）`,
        next: `弹射 ${level + 1} 次（70% 伤害）`,
      };
    default:
      return null;
  }

...
> **逻辑层规范（2026-09-13）**：所有数值/结算逻辑集中在 `src/logic/` 纯函数模块，按类分文件（wave.ts 难度曲线 / affix.ts 词缀结算 / player.ts 玩家被动…），函数统一 `calc` 前缀、零 Phaser 依赖、可单测。业务模块（entities/systems/data）**禁止内联公式**，只能 import 调用；新增公式先写进 logic + 测试，再接入调用点。测试断言浮点一律 `toBeCloseTo`（实测 0.1×n 会产生 2.9000000000000004 类误差）。

> **测试约定（2026-09-13）**：凡改动/新增落在 Vitest 可管范围——数值公式（被动/词缀/伤害结算等）、数据表结构（affixes/upgrades/achievements…）、状态/流程逻辑（存档恢复/对局生命周期）、时间逻辑（限时状态）、行为验证——必须同步补对应单测（tests/*.test.ts），跑 `npm test` 全绿后再提交。纯 UI 布局/渲染表现不在 Vitest 范围，用双端（`?mobile=1` 与桌面）手动验证。
...
### 🚀 远期方向 / 点子库（2026-09-12 用户提出，未排期，仅记录）

**1. 多人同步对战（世界级 Boss）**
- WebSocket 长连接 + 后台服务器 + 账号系统（含游客登录：本地生成匿名 ID，后续可绑定）
- 玩法设想：多名玩家同场共斗，共同抵抗"世界级 Boss"（共享血条 / 同步伤害 / 全局事件）
- 技术点：状态同步（玩家位置/弹幕/伤害结算权威在服务端或混合）、断线重连、延迟补偿

**2. 后台管理页**
- 统计游玩数据：局数 / 时长 / 击杀 / 波次分布 / 胜率 / 成就解锁率 / 留存
- 参数热更：部分游戏内数值由后台下发（不随包发布），便于线上调平衡

**3. 地图与战斗深度**
- 地图复杂度：当前大方块无策略感 → 区块化设计 / 引入**随机地图**（种子生成：地形、障碍、宝箱、事件点分布）
- 敌人密度与弹幕：正常游玩下弹幕威胁不足 → 增加敌人种类与密度曲线、弹幕 pattern（密度/速度/轨迹）
- 主动技能：如**闪避**（位移 + 无敌帧），需配套 CD / 资源（体力）设计，避免无限风筝

**4. 账号系统接入后的模式调整**
- 成就等进度随账号保留
- **无尽模式取消或改为单独入口**：保证每一局正常游玩都有"探索意义"（目标导向的局内探索：事件 / 房间 / 收集 / 阶段性目标），而非纯挂机刷波次
...
- 已完成：数值收敛第一刀（2026-09-13）——超限强化四件套线性化（攻击 +3 / 暴击率 +1% / 暴伤 +0.5% / 生命 +20，替代 ×1.03·+3%·×1.05 指数），公式入 src/logic/player.ts + 4 例单测；正常局数值健康无需动，深度平衡（敌人成长/武器乘区）留待后续
...
- ✅ 词缀爆炸距离衰减（已完成 2026-09-13）：Enemy 词缀爆炸接线 calcExplodePlayerDamage（近身全额、边缘 50%，与自爆怪 falloff 一致），公式测试已全覆盖（贴脸/边缘/半程/超半径兜底）
  - 分析备忘（2026-09-12，已实施前）：已排除多词缀叠加/自爆+爆炸叠加/Boss 挂词缀/爆炸对敌人连锁；唯一风险点即"爆炸词缀全额无距离衰减"，后期范围武器一次清 3-4 只爆炸怪叠加吃满——不会秒但会疼
...
## 🎨 UI 风格规范（2026-09-13 定稿，UIStyle.ts）

### 背景
主菜单 / 选角 / 选关 / 敌方情报四个菜单类界面原来各玩各的：标题字号 56/34/32/28 无规范、背景三种深色、面板卡片底色五个变体、灰色文字五种（#888888/#8a8a8a/#777777/#8a8a99/#8888aa）、返回按钮文案「‹ 返回」与「← 返回」混用、设置面板甚至用青色（0x00ffff）描边。

### 规范内容（src/ui/UIStyle.ts，新场景一律引用，禁止散落魔法颜色/字号）
- **唯一强调色**：橙色 #ff6b35（标题 / 选中 / hover / 描边）
- **背景**：场景 0x0a0a12（图鉴类 0x0a0a14）；面板统一 0x16161f + 2px 橙描边(alpha 0.4) + 圆角 14 → `createUIPanel`
- **文字三级**：主 #e0e0e0 / 次 #8a8a99 / 弱 #666677（语义色：绿增益 / 蓝信息 / 金徽章 / 红减益）
- **标题字号**：场景 34 / 面板 28 / 区块 16；按钮 18（主菜单大按钮 24）；正文 18 / 说明 15 / 小字 13 / 微字 12
- **标准组件**：`createSceneTitle`（34px 橙加粗）/ `createBackButton`（统一「← 返回」）/ `createUIButton`（深底 + hover 橙字 + 点击音效 + 缩放反馈）/ `createUIPanel` / `styleTab`（选项卡 active 橙粗体）
- **按钮底色**：通用/场景按钮 #1a1a25（hover #2a2a35）；面板内按钮亮底 #252530；锁定 #1a1a22；图鉴/信息类蓝调底 #1a1a35

### 已收敛场景（2026-09-13）
- CharacterSelectScene：背景/标题/卡片三态/返回/开始冒险 ✓
- EnemyCodexScene：背景/标题/返回/选项卡/格子三态/两处详情面板/全部文字色 ✓（styleTab 收编选项卡高亮）
- MainMenuScene：副标题/6 菜单按钮/底部统计/版本号/更新提示/选关面板（面板+标题+关卡按钮+感叹号+预览行+关闭）/设置面板（青→橙统一）/createSmallButton/画质高亮/开关状态色 ✓
- 待收敛：成就、商店、结算、突破、玩家属性等其余菜单类场景（同一令牌，后续逐个换）

### 顺带修复的隐藏 bug（2026-09-13）
- 主菜单「开始游戏」与「继续游戏」y 坐标都写成 buttonY+spacing → 两按钮重叠、视觉只剩 5 个按钮、点到的实际是「继续游戏」（会直接恢复存档进游戏）。已改为 +1/+2/+3/+4/+5 递增。
- 这解释了用户此前「为什么我没看到开始游戏按钮」的疑惑（当时误判在选角界面）。

### 排版铁律（沿用「后续 UI 调整 checklist」）
- 对称 / 合理留白 / 元素联动：先画几何骨架再放内容；附属控件贴主控件边缘（间隙 ≤10-20px）；避免元素重叠与空白过大过小

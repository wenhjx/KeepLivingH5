...
## 八、数值审计工具（方案 A，秒级静态验证）

**用途**：不用开浏览器、不用挂机自动玩，直接复用游戏数据配置做确定性计算，秒出数值曲线与风险结论。
改完数值（敌人/武器/关卡/经济）后跑一次即可快速看影响。

```bash
npm run audit
# 控制台输出表格 + 生成 scripts/balance-report.html 可视化报告
```

**输出内容**：敌人 HP/攻击/移速曲线（含全局×1.3 系数）、Boss 血量与击杀需几波、武器 DPS 排行、升级经验曲线、每波经济、三关对比、坎检测结论。

**公式与源码对应**（改公式前先看脚本头部注释）：
- 敌人血量 = 基础 × 波次难度 × 关卡倍率 × 1.3（Enemy.spawn）
- 普通敌人难度 = 1+(波-1)×0.1；Boss 难度 = 2.2^(tier-1)（WaveManager）
- 武器伤害 = damage × (1+等级×0.2) × 攻击力/10（Player）
- 经验需求 = floor(20 × 等级^1.25)（GameConfig.LEVEL）
- 生成间隔 = max(200, 1500-波×50)ms，每波 30s

**动态因素均值假设**：命中率 70%、穿透/AOE 平均命中目标数（脚本 WEAPON_HIT_TARGETS）、暴击期望系数、同屏上限取 medium 120。
**已知局限**：不含走位/弹幕躲避/消耗品（狂暴药水 +50%、炸弹 500 伤、复活币）的实际影响——审计判定"危"的场景需用自动游玩抽查复核。
**维护约定**：游戏数值公式改动后同步更新脚本头部注释与公式，避免审计与实机脱节。
